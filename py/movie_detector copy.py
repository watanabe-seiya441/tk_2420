...
def annotate_video(input_video_path: str, output_path: str, model_path: str = "hackday/models/hackv4i.pt") -> None:
    ...
    class_names_list = ["karina", "giselle", "winter", "ningning"]
    for class_name in class_names_list:
        overlay_data[class_name] = {
            "id": class_name,
            "content": class_name,
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        frame_count += 1
        current_time = round((frame_count - 1) / fps, 2)  # 時間を計算
            ...
            for box in result.boxes:
                ...
                if not confidence_low_flag and tracking_id is not None:
                    ...
                    if class_name in overlay_data:
                        ...
                        overlay_data[class_name]["positions"].append(position)
    ...
    output_json = {"overlays": list(overlay_data.values())}
...
def save_photo(frame, frame_width, frame_height, x1, y1, x2, y2, class_name):
    # 必要に応じてsave_photo関数を実装
    pass
