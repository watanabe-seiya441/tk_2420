    ...
    with torch.no_grad():
        for images, idx in tqdm(dataloader):
            ...
            if use_flip_test:
                ...
                fused_feature, fused_norm = fuse_features_with_norm(
                    stacked_embeddings, stacked_norms, fusion_method=fusion_method
                )
...
def identification(data_root, dataset_name, img_input_feats, save_path):
    ...
    gallery_templates_feature, gallery_unique_templates, gallery_unique_subject_ids = (
        eval_helper_identification.image2template_feature(
    ...
    probe_mixed_templates_feature, probe_mixed_unique_templates, probe_mixed_unique_subject_ids = (
        eval_helper_identification.image2template_feature(
    ...
    pd.DataFrame(identification_result, index=["identification"]).to_csv(
        os.path.join(save_path, "identification_result.csv")
    )
...
def verification(data_root, dataset_name, img_input_feats, save_path):
    ...
    p1, p2, label = eval_helper_verification.read_template_pair_list(
        os.path.join(data_root, "%s/meta" % dataset_name, "%s_template_pair_label.txt" % dataset_name.lower())
    )

    template_norm_feats, unique_templates = eval_helper_verification.image2template_feature(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fusion_method",
        type=str,
        default="pre_norm_vector_add",
        choices=("average", "norm_weighted_avg", "pre_norm_vector_add", "concat"),
    )
    ...
    landmark_list_path = os.path.join(
        args.data_root, "./%s/meta/%s_name_5pts_score.txt" % (dataset_name, dataset_name.lower())
    )
    img_input_feats, faceness_scores, norms = infer_images(
        model=model,
        img_root=img_root,
