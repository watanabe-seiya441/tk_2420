    ...
    def prepare_data(self):
        ...
        if not os.path.isfile(self.concat_mem_file_name):
            ...
            for key in ["agedb_30", "cfp_fp", "lfw", "cplfw", "calfw"]:
                np_array, issame = evaluate_utils.get_val_pair(
                    path=os.path.join(self.data_root, self.val_data_path), name=key, use_memfile=False
                )
...
def train_dataset(
    ...
    if use_mxrecord:
        ...
        train_dataset = AugmentRecordDataset(
            root_dir=train_dir,
