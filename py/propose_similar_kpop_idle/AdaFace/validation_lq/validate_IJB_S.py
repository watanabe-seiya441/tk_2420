...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fuse_match_method", type=str, default="pre_norm_vector_add_cos", choices=("pre_norm_vector_add_cos")
    )
    ...
    for partition_idx in tqdm(range(num_partition)):
        ...
        if args.save_features:
            save_path = os.path.join(
                save_root, "feature_extracted/ijbs_pred_{}_{}.npy".format(args.model_name, partition_idx)
            )
        ...
        if len(norms) > 0:
            ...
            if args.save_features:
                save_path = os.path.join(
                    save_root, "feature_extracted/ijbs_pred_{}_norm_{}.npy".format(args.model_name, partition_idx)
                )
    ...
    # evaluate
    evaluate_helper.run_eval_with_features(
        save_root=save_root,
