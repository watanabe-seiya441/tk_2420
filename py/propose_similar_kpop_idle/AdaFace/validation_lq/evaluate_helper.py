...
def eval_IJBS(
    ...
    if save_root is not None:
        write_result(
            ...
            title="surveillance_to_single",
            values=["rank1", "rank5", "rank10"],
        )
        write_result(
            os.path.join(save_root, "eval_result/{}_result.csv".format(fuse_match_method)),
            title="closedset",
            values=DIRs_closeset.tolist(),
        )
        write_result(
            os.path.join(save_root, "eval_result/{}_result.csv".format(fuse_match_method)),
            title="surveillance_to_single",
            values=["0.01_FPIR", "0.1_FPIR"],
        )
        write_result(
            os.path.join(save_root, "eval_result/{}_result.csv".format(fuse_match_method)),
            title="openset",
            values=DIRs_openset.tolist(),
        )
    print(DIRs_closeset)
    print(DIRs_openset)
    ...
    if save_root is not None:
        write_result(
            os.path.join(save_root, "eval_result/{}_result.csv".format(fuse_match_method)),
            title="multiview_surveillance_to_booking",
            values=["rank1", "rank5", "rank10"],
        )
        write_result(
            os.path.join(save_root, "eval_result/{}_result.csv".format(fuse_match_method)),
    ...
    if save_root is not None:
        write_result(
            os.path.join(save_root, "eval_result/{}_result.csv".format(fuse_match_method)),
            title="surveillance_to_surveillance",
            values=["rank1", "rank5", "rank10"],
        )
        write_result(
            os.path.join(save_root, "eval_result/{}_result.csv".format(fuse_match_method)),
...
def run_eval_with_features(
    save_root, features, image_paths, get_retrievals=False, fuse_match_method="mean_cos", ijbs_proto_path=None
):
    assert len(features) == len(image_paths)

    all_result = eval_IJBS(
        feat_func=features,
