...
class TinyFaceTest:
    def __init__(self, tinyface_root="/data/data/faces/tinyface", alignment_dir_name="aligned_pad_0.1_pad_high"):
        ...
        self.proto_gal_paths = [
            os.path.join(tinyface_root, alignment_dir_name, "Gallery_Match", p[0].item())
            for p in self.gallery_dict["gallery_set"]
        ]
        self.proto_prob_paths = [
            os.path.join(tinyface_root, alignment_dir_name, "Probe", p[0].item()) for p in self.probe_dict["probe_set"]
        ]
