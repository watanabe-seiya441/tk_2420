...
class IJBSTest:
    ...
    def init_proto(self, folder_ijbs_protocols):
        ...
        video_meta_list = [
            (x.split(",")[0], x.split(",")[1]) for x in meta_lines if x.split(",")[1].startswith("videos")
        ]
        video_meta_list = list(
            set(video_meta_list)
        )  # list of (person_id, video_name) there are total 398 videos, but multiple people pop up in the video. so there are 3852 pairs
        ...
        filename = os.path.join(folder_ijbs_protocols, "galleries/cs6_surveillance_to_surveillance_g1.csv")
        with open(filename, "r") as f:
            lines = f.readlines()
            lines = lines[1:]
        video_list = [(x.split(",")[1], x.split(",")[2]) for x in lines]
        video_list = list(set(video_list))
        self.protocol_template_indices["video_gallery1"] = [video_meta_list.index(x) for x in video_list]
        self.protocol_template_indices["video_gallery1"] = (
            np.array(self.protocol_template_indices["video_gallery1"]).astype(int).flatten()
        )

        # getting 101 video list as 'video_gallery2' g2
        filename = os.path.join(folder_ijbs_protocols, "galleries/cs6_surveillance_to_surveillance_g2.csv")
        with open(filename, "r") as f:
            lines = f.readlines()
            lines = lines[1:]
        video_list = [(x.split(",")[1], x.split(",")[2]) for x in lines]
        video_list = list(set(video_list))
        self.protocol_template_indices["video_gallery2"] = [video_meta_list.index(x) for x in video_list]
        self.protocol_template_indices["video_gallery2"] = (
            np.array(self.protocol_template_indices["video_gallery2"]).astype(int).flatten()
        )
        ############ Get Video Gallery Templates ################
        ...
        filename = os.path.join(folder_ijbs_protocols, "cs6_surveillance_to_single-booking_probe.csv")
        with open(filename, "r") as f:
            lines = f.readlines()
            lines = lines[1:]
        video_list = [x.split("\n")[0] for x in lines]
        self.protocol_template_indices["video2image_probe"] = [
            x[0] for x in enumerate(video_meta_list) if x[1][1] in video_list
        ]
        self.protocol_template_indices["video2image_probe"] = (
            np.array(self.protocol_template_indices["video2image_probe"]).astype(int).flatten()
        )
        ############ Get Video to Image Probe Templates ################

        ############ Get Video to Video Probe Templates ################
        print("Get Video to Video Probe Templates......")
        filename = os.path.join(folder_ijbs_protocols, "cs6_surveillance_to_surveillance_probe.csv")
        with open(filename, "r") as f:
            lines = f.readlines()
            lines = lines[1:]
        ...
        self.protocol_template_indices["video2video_probe"] = (
            np.array(self.protocol_template_indices["video2video_probe"]).astype(int).flatten()
        )

        # 2. Multi-view Surveillance to booking
        self.protocol_template_indices["multiview_video_probe"] = []
        subject_list = [
            self.all_template_list[x].subject_id for x in self.protocol_template_indices["video2video_probe"]
        ]
        ...
        for subject in subject_list:
            ...
            template.videos = [video_meta_list[x][1] for x in subject_video_dict[subject]]
            template.template_indices = np.array(subject_video_dict[subject]).astype(int).flatten()
            self.all_template_list.append(template)
            self.protocol_template_indices["multiview_video_probe"].append(index)
            index += 1
        self.protocol_template_indices["multiview_video_probe"] = (
            np.array(self.protocol_template_indices["multiview_video_probe"]).astype(int).flatten()
        )
        ############ Get Video to Video Probe Templates ################
        ...
        filename = os.path.join(folder_ijbs_protocols, "cs6_uav_to_single-booking_probe.csv")
        with open(filename, "r") as f:
            lines = f.readlines()
            lines = lines[1:]
        uav_list = [x.split("\n")[0] for x in lines]
        ...
        for uav in uav_list:
            if uav.startswith("videos"):
                ...
                template = IJBSTemplate()
                template.subject_id = [int(x.split(",")[0]) for x in meta_lines if x.split(",")[1] == uav][0]
                template.images = [uav]
                self.all_template_list.append(template)
                self.protocol_template_indices["uav_probe"].append(index)
                index += 1
        self.protocol_template_indices["uav_probe"] = (
            np.array(self.protocol_template_indices["uav_probe"]).astype(int).flatten()
        )
        ############ Get UAV Video Probe Templates ################
        ...
        filename = os.path.join(folder_ijbs_protocols, "galleries/cs6_surveillance_to_single_g1.csv")
        with open(filename, "r") as f:
            lines = f.readlines()
            lines = lines[1:]
        ...
        for image in image_list:
            ...
            index += 1
        self.protocol_template_indices["single_image_gallery1"] = (
            np.array(self.protocol_template_indices["single_image_gallery1"]).astype(int).flatten()
        )

        self.protocol_template_indices["single_image_gallery2"] = []
        filename = os.path.join(folder_ijbs_protocols, "galleries/cs6_surveillance_to_single_g2.csv")
        with open(filename, "r") as f:
            lines = f.readlines()
            lines = lines[1:]
        image_list = [(x.split(",")[1], x.split(",")[2]) for x in lines]
        for image in image_list:
            ...
            index += 1
        self.protocol_template_indices["single_image_gallery2"] = (
            np.array(self.protocol_template_indices["single_image_gallery2"]).astype(int).flatten()
        )
        ############ Get Single Image Templates ################
        ...
        filename = os.path.join(folder_ijbs_protocols, "galleries/cs6_surveillance_to_booking_g1.csv")
        with open(filename, "r") as f:
            lines = f.readlines()
            lines = lines[1:]
        ...
        for subject in subject_list:
            template = IJBSTemplate()
            template.subject_id = subject
            template.images = [x.split(",")[2] for x in lines if int(x.split(",")[1]) == subject]
            self.all_template_list.append(template)
            self.protocol_template_indices["booking_image_gallery1"].append(index)
            index += 1
        self.protocol_template_indices["booking_image_gallery1"] = (
            np.array(self.protocol_template_indices["booking_image_gallery1"]).astype(int).flatten()
        )

        self.protocol_template_indices["booking_image_gallery2"] = []
        filename = os.path.join(folder_ijbs_protocols, "galleries/cs6_surveillance_to_booking_g2.csv")
        with open(filename, "r") as f:
            lines = f.readlines()
            lines = lines[1:]
        subject_list = [int(x.split(",")[1]) for x in lines]
        ...
        for subject in subject_list:
            ...
            index += 1
        self.protocol_template_indices["booking_image_gallery2"] = (
            np.array(self.protocol_template_indices["booking_image_gallery2"]).astype(int).flatten()
        )
        ############ Get Booking Image Templates ################

        self.all_template_list = np.array(self.all_template_list).astype(np.object).flatten()
        ...
        for i, template in enumerate(self.all_template_list):
            ...
            if template.videos is not None:
                for video in template.videos:
                    temp = video.split("/")
                    name = str(template.subject_id) + "_" + temp[0] + "_" + os.path.splitext(temp[1])[0]
    ...
    def identification(self, template_probe, template_gallery1, template_gallery2, get_retrievals=False):
        ...
        DIRs_closeset, _, _ = DIR_FAR(
            np.hstack((scores1, scores2)),
            label_probe == np.vstack((label_gallery1, label_gallery2)).T,
            ranks=[1, 5, 10],
        )
        ...
        # scores4 = scores3.T
        DIRs_openset1, _, _ = DIR_FAR(scores1, label_probe == label_gallery1.T, FARs=[0.01, 0.1])
        DIRs_openset2, _, _ = DIR_FAR(scores2, label_probe == label_gallery2.T, FARs=[0.01, 0.1])
        DIRs_openset = (DIRs_openset1 + DIRs_openset2) / 2.0

        # return DIRs_closeset, DIRs_openset

        if get_retrievals:
            _, _, _, mate_indices, success, sort_idx_mat_m, sorted_score_mat_m = DIR_FAR(
                np.hstack((scores1, scores2)),
                label_probe == np.vstack((label_gallery1, label_gallery2)).T,
                ranks=[1],
                get_retrievals=True,
    ...
    def surveillance_to_single(self, get_retrievals=False):
        template_probe = self.all_template_list[self.protocol_template_indices["video2image_probe"]]
        template_gallery1 = self.all_template_list[self.protocol_template_indices["single_image_gallery1"]]
        template_gallery2 = self.all_template_list[self.protocol_template_indices["single_image_gallery2"]]
        return self.identification(template_probe, template_gallery1, template_gallery2, get_retrievals=get_retrievals)

    def surveillance_to_booking(self, get_retrievals=False):
        template_probe = self.all_template_list[self.protocol_template_indices["video2image_probe"]]
        template_gallery1 = self.all_template_list[self.protocol_template_indices["booking_image_gallery1"]]
        template_gallery2 = self.all_template_list[self.protocol_template_indices["booking_image_gallery2"]]
        return self.identification(template_probe, template_gallery1, template_gallery2, get_retrievals=get_retrievals)

    def multiview_surveillance_to_booking(self, get_retrievals=False):
        template_probe = self.all_template_list[self.protocol_template_indices["multiview_video_probe"]]
        template_gallery1 = self.all_template_list[self.protocol_template_indices["booking_image_gallery1"]]
        template_gallery2 = self.all_template_list[self.protocol_template_indices["booking_image_gallery2"]]
        return self.identification(template_probe, template_gallery1, template_gallery2, get_retrievals=get_retrievals)

    def surveillance_to_surveillance(self, get_retrievals=False):
        template_probe = self.all_template_list[self.protocol_template_indices["video2video_probe"]]
        template_gallery1 = self.all_template_list[self.protocol_template_indices["video_gallery1"]]
        template_gallery2 = self.all_template_list[self.protocol_template_indices["video_gallery2"]]
        return self.identification(template_probe, template_gallery1, template_gallery2, get_retrievals=get_retrievals)

    def uav_surveillance_to_booking(self, get_retrievals=False):
        template_probe = self.all_template_list[self.protocol_template_indices["uav_probe"]]
        template_gallery1 = self.all_template_list[self.protocol_template_indices["booking_image_gallery1"]]
        template_gallery2 = self.all_template_list[self.protocol_template_indices["booking_image_gallery2"]]
        return self.identification(template_probe, template_gallery1, template_gallery2, get_retrievals=get_retrievals)
