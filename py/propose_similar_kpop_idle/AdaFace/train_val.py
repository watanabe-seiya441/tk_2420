...
class Trainer(LightningModule):
    def __init__(self, **kwargs):
        ...
        if self.hparams.start_from_model_statedict:
            ...
            self.model.load_state_dict(
                {key.replace("model.", ""): val for key, val in ckpt["state_dict"].items() if "model." in key}
            )
    ...
    def validation_epoch_end(self, outputs):
        ...
        val_logs["val_acc"] = np.mean(
            [
                val_logs[f"{dataname}_val_acc"]
                for dataname in dataname_to_idx.keys()
                if f"{dataname}_val_acc" in val_logs
            ]
        )
    ...
    def gather_outputs(self, outputs):
        ...
        for _out, _nor, _tar, _dat, _idx in zip(
            all_output_tensor, all_norm_tensor, all_target_tensor, all_dataname_tensor, all_image_index
        ):
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.SGD(
            [{"params": paras_wo_bn + [self.head.kernel], "weight_decay": 5e-4}, {"params": paras_only_bn}],
            lr=self.hparams.lr,
            momentum=self.hparams.momentum,
        )

        scheduler = lr_scheduler.MultiStepLR(
            optimizer, milestones=self.hparams.lr_milestones, gamma=self.hparams.lr_gamma
        )
