...
const EnhancedVideoPlayer: React.FC<EnhancedVideoPlayerProps> = ({
  ...
  useEffect(() => {
    ...
    fetch(overlayConfigUrl)
      ...
      .then((data) => {
        ...
        const initialVisibility = data.overlays.reduce(
          (acc: { [key: string]: boolean }, overlay: Overlay) => {
  ...
  useEffect(() => {
    const updateOverlayPositions = () => {
      ...
      const newPositions = overlays.reduce(
        (acc, overlay) => {
    ...
    <div ref={containerRef} className="relative w-full max-w-3xl mx-auto">
      ...
      {isOverlayVisible &&
        overlays.map((overlay) => {
          ...
          const isIndividualOverlayVisible =
            individualVisibility[overlay.content];
